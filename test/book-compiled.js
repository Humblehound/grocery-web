  ...
  */
	describe('/GET book', function () {
		it('it should GET all the books', function (done) {
			chai.request(server).get('/book').end(function (err, res) {
  ...
  */
	describe('/POST book', function () {
		it('it should not POST a book without pages field', function (done) {
			...
			chai.request(server).post('/book').send(book).end(function (err, res) {
				res.should.have.status(200);
	...
	describe('/GET/:id book', function () {
		it('it should GET a book by the given id', function (done) {
			...
			book.save(function (err, book) {
				chai.request(server).get('/book/' + book.id).send(book).end(function (err, res) {
					res.should.have.status(200);
					res.body.should.be.a('object');
  ...
  */
	describe('/PUT/:id book', function () {
		it('it should UPDATE a book given the id', function (done) {
			...
			book.save(function (err, book) {
				chai.request(server).put('/book/' + book.id).send({ title: "The Chronicles of Narnia", author: "C.S. Lewis", year: 1950, pages: 778 }).end(function (err, res) {
					res.should.have.status(200);
					res.body.should.be.a('object');
  ...
  */
	describe('/DELETE/:id book', function () {
		it('it should DELETE a book given the id', function (done) {
			var book = new Book({ title: "The Chronicles of Narnia", author: "C.S. Lewis", year: 1948, pages: 778 });
			book.save(function (err, book) {
				chai.request(server).delete('/book/' + book.id).end(function (err, res) {
					res.should.have.status(200);
					res.body.should.be.a('object');
